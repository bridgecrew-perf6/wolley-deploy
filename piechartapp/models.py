...
class PieChart(models.Model):
    id = models.BigAutoField(primary_key=True, db_column="piechart_id")
    user = models.ForeignKey(AppUser, on_delete=models.CASCADE, related_name="piecharts")  # user가 piechart를 여러 개 가질 수 있다.
    ...
    def __str__(self):
        return f'{self.user} -> piechart {self.id}'
